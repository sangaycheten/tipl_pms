...
class AuthController extends Controller
{
	...
	$redirect = Input::get('redirect');
        ...
        $authenticated = Auth::attempt(['EmpId' => $empId, 'password' => $password, 'Status' => 1], Input::has('RememberMe'));

        if (!$authenticated) {
            $authenticated = Auth::attempt(['EmpId' => $empId, 'password' => $password, 'Status' => 3], Input::has('RememberMe'));
        }
