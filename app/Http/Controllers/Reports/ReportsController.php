...
class ReportsController extends Controller
{
    public function getPMSComparisionEmployees()
    {
        ...
        $gradeStep = request('GradeStep');
        $pmsPeriod = request('PMSPeriod');
        ...
        if (!empty($result)) {
            if (request()->has('export') && request('export') == 'excel') {
                return Excel::download(
                    new \App\Exports\PmsScoreReportExport(['outcomes' => $outcomes, 'result' => $result, 'employees' => isset($employees) ? $employees : [], 'pmsPeriodArray' => $pmsPeriodArray, 'pmsPeriods' => $pmsPeriods, 'departments' => $departments, 'sections' => $sections]),
                    'PMS Result_' . date('Y_m_d_H_i_s') . '.xlsx'
                );
            }
        }
    ...
    public function getEligibleForMeritorious(Request $request)
    {
        ...
        if (request()->has('export') && request('export') == 'excel') {
            return Excel::download(
                new \App\Exports\EligibleForMeritoriousExport(['outstandingAndGoodEligible' => $outstandingAndGoodEligible, 'outstandingEligible' => $outstandingEligible]),
                'Eligible_for_Meritorious_' . date('Y_m_d_H_i_s') . '.xlsx'
            );
        }
    ...
    public function getEligibleForLoa(Request $request)
    {
        ...
        $threeRoundsIncludingCurrentString = implode(",", $threeRoundsIncludingCurrent->toArray());
        ...
        if (request()->has('export') && request('export') == 'excel') {
            return Excel::download(
                new \App\Exports\EligibleForLoaExport(['loaEligible' => $loaEligible]),
                'Eligible_for_LoA_' . date('Y_m_d_H_i_s') . '.xlsx'
            );
        }
    ...
    public function getEligibleForRegular(Request $request)
    {
        ...
        if (request()->has('export') && request('export') == 'excel') {
            return Excel::download(
                new \App\Exports\EligibleForRegularExport(['regularPromotionEligible' => $regularPromotionEligible, 'regularPromotionEligibleIds' => $regularPromotionEligibleIds]),
                'Eligible_for_Regular_' . date('Y_m_d_H_i_s') . '.xlsx'
            );
        }
    ...
    public function getLowPerformingEmployees(Request $request)
    {
        ...
        $twoRoundsIncludingCurrentString = implode(",", $twoRoundsIncludingCurrent->toArray());
        ...
        $oneRoundIncludingCurrentString = implode(",", $oneRoundIncludingCurrent->toArray());
        //END
