...
class GoalController extends Controller
{
    public function getList()
    {
        ...
        if (!$withinSecondPMSOfYear && !$withinFirstPMSOfYear) {
            ...
            if ($today > strtotime(date(CONST_PMSSETTING_SECONDPMSENDDATE)) && $today <= strtotime(date(CONST_PMSSETTING_FIRSTPMSENDDATE))) {
        ...
        if ($userPositionId == CONST_POSITION_HOS) {
            $type = 1;
            $employees = DB::select("SELECT T1.Id, concat(T1.Name,', ',V.ShortName,' Department') as Name, T2.PMSOutcomeId, T2.Id as PMSSubmissionId, P.Id as GoalDefinitionId,P.Status as GoalDefinitionStatus, O.Name as Designation, Z.Name as Position, T2.LastStatusId, T3.Name as Status, T2.Id as SubmissionId, T4.SubmittedAt from (mas_employee T1 join mas_department V on V.Id = T1.DepartmentId join mas_designation O on O.Id = T1.DesignationId left join mas_hierarchy B on B.EmployeeId = T1.Id) left join pms_employeegoal P on P.EmployeeId = T1.Id and P.SysPmsNumberId = ? join mas_gradestep Z on Z.Id = T1.GradeStepId left join mas_position A on A.Id = T1.PositionId left join (viewpmssubmissionwithlaststatus T2 join mas_pmsstatus T3 on T3.Id = T2.LastStatusId) on T2.EmployeeId = T1.Id and T2.SubmissionTIme >= ? LEFT JOIN pms_employeegoal_l1submission T4 ON P.Id = T4.EmployeeGoalId where coalesce(T1.Status,0) = 1 and (B.ReportingLevel1EmployeeId = ? or B.Reportinglevel2EmployeeId = ?) order by A.DisplayOrder,T1.Name", [$currentPMSNumber, $currentPMSStartDate, Auth::user()->Id, Auth::user()->Id]);
        ...
        return view('goals.employeelist')->with('currentRound', $currentRound)
                ->with('withinFirstPMSOfYear', $withinFirstPMSOfYear)
                ->with('withinSecondPMSOfYear', $withinSecondPMSOfYear)
                ->with('currentPmsPeriod', $currentPmsPeriod)
                ->with('notWithinPMSPeriod', $notWithinPMSPeriod)
                ->with('type', $type)
                ->with('units', $units)
    ...
    public function fetchSubordinateGoals(Request $request)
    {
        ...
        $query = DB::table('pms_employeegoaldetail as T2')
        ...
        if ((bool) $goal->MultipleLevel1Appraiser) {
            ...
            $result = $query
                ->join(
                    'pms_employeegoaltargetdetail as T3',
                    'T2.Id','=','T3.GoalDetailId'
                )
                ->join(
            ...
            $result = $query
                ...
                ->selectRaw('
                    T2.Id,
                    T2.Description as GoalDescription,
                    T2.DisplayOrder,
                    T2.Weightage as GoalWeightage,
                    T2.Target as GoalTarget,
                    T3.Description as TargetDescription,
                    T3.Weightage as TargetWeightage,
                    T3.Target as TargetValue,
                    T3.SelfScore,
                    T3.SelfRemarks,
                    T3.Level1Score,
                    T3.Level1Remarks,
