...
class PMSController extends Controller
{
    public function getIndex()
    {
        ...
        if ($today >= strtotime(date(CONST_PMSSETTING_SECONDPMSSTARTDATE)) && $today <= strtotime(date(CONST_PMSSETTING_SECONDPMSENDDATE))) {
            ...
            $pmsCount = DB::table('pms_submission')->where('EmployeeId', Auth::user()->Id)->whereRaw("(DATE_FORMAT(SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(SubmissionTime,'%Y-%m-%d') <= ?)", [date(CONST_PMSSETTING_SECONDPMSSTARTDATE), date(CONST_PMSSETTING_SECONDPMSENDDATE)])->count();
            $lastPMSQuery = DB::table('pms_submission')->where('EmployeeId', Auth::user()->Id)->whereRaw("(DATE_FORMAT(SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(SubmissionTime,'%Y-%m-%d') <= ?)", [date(CONST_PMSSETTING_SECONDPMSSTARTDATE), date(CONST_PMSSETTING_SECONDPMSENDDATE)])->pluck('Id');
            ...
            if ($today >= strtotime(date(CONST_PMSSETTING_FIRSTPMSSTARTDATE)) && $today <= strtotime(date(CONST_PMSSETTING_FIRSTPMSENDDATE))) {
                ...
                $pmsCount = DB::table('pms_submission')->where('EmployeeId', Auth::user()->Id)->whereRaw("(DATE_FORMAT(SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(SubmissionTime,'%Y-%m-%d') <= ?)", [date(CONST_PMSSETTING_FIRSTPMSSTARTDATE), date(CONST_PMSSETTING_FIRSTPMSENDDATE)])->count();
                $lastPMSQuery = DB::table('pms_submission')->where('EmployeeId', Auth::user()->Id)->whereRaw("(DATE_FORMAT(SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(SubmissionTime,'%Y-%m-%d') <= ?)", [date(CONST_PMSSETTING_FIRSTPMSSTARTDATE), date(CONST_PMSSETTING_FIRSTPMSENDDATE)])->pluck('Id');
                $lastPMSId = isset($lastPMSQuery[0]) ? $lastPMSQuery[0] : '';
            }
        }
        ...
        $activeCycle  = $currentMonth === PMS_H1_START_MONTH ? 'H1'
                        : ($currentMonth === PMS_H2_START_MONTH ? 'H2' : null);
            ...
            if (Auth::user()->DepartmentId == 7) {
                $goalAchievementScore = DB::table("pms_employeegoal as T1")
                    ...
                    ->where(function ($query) use ($activeCycle) {
                        $query->where('T1.Status', 1) // for old code compatibility where status column was used to identify finalized record
                              ->orWhere('T1.' . $activeCycle . 'Status', 1); // considering H1Status as 1 means the record is finalized for H1 or H2
                    })
                    ->sum("T2.SelfScore");
        ...
        return view('application.pmsindex', 
            [
                'goalAchievementScore' => $goalAchievementScore, 
                'noOfEmployees' => $noOfEmployees, 
                'status' => $status, 
                'scoresOfSubordinate' => $scoresOfSubordinate, 
                'subordinateScorePercentage' => $subordinateScorePercentage, 
                'alreadySubmitted' => $alreadySubmitted, 
                'hasWeightageDefined' => $hasWeightageDefined, 
                'hasAssessmentAreasDefined' => $hasAssessmentAreasDefined, 
                'notWithinPMSPeriod' => $notWithinPMSPeriod, 
    ...
    public function postUploadExcelApplicant()
    {
        ...
        if (in_array($extension, ['xls', 'xlsx'])) {
            $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($excelFile->getPathname());
            $lastSheet = $spreadsheet->getSheet($spreadsheet->getSheetCount() - 1);
            $data = $lastSheet->toArray();
            $finalResult = 0;
            foreach ($data as $row) {
            ...
            return response()->json(['success' => true, 'score' => number_format((float) $finalResult, 3)]);
        } else {
    ...
    public function getAppraise()
    {
        ...
        if ($today >= strtotime(date(CONST_PMSSETTING_SECONDPMSSTARTDATE))) {
            $finalAdjustmentPercent = DB::table('mas_pmssettings')->whereRaw('created_at >= ?', [date(CONST_PMSSETTING_SECONDPMSSTARTDATE . ' 00:00:00')])->value('FinalAdjustmentPercent');
            $fromDate = date(CONST_PMSSETTING_SECONDPMSSTARTDATE);
            $toDate = date(CONST_PMSSETTING_SECONDPMSENDDATE);
        } else {
            $finalAdjustmentPercent = DB::table('mas_pmssettings')->whereRaw('created_at >= ? and created_at <= ?', [date(CONST_PMSSETTING_FIRSTPMSSTARTDATE . ' 00:00:00'), date(CONST_PMSSETTING_SECONDPMSSTARTDATE . ' 23:59:59')])->value('FinalAdjustmentPercent');
        ...
        //     // This block will run for your current collection
        //     return "There are no items to display.";
        // }

        // dd($finalAdjustmentPercent);

        return view('application.pmsappraise')->with('isAppraiser', $isAppraiser)
            ->with('notWithinPMS', $notWithinPMS)
            ->with('totalEmpCount', $totalEmpCount)
            ->with('finalScoreNotApplied', $this->checkAllPMSHasFinalScore())
            ->with('pmsNotSubmitted', $pmsNotSubmitted)
            ->with('pmsNotCompleted', $pmsNotCompleted)
            ->with('revDetails', $revDetails)
            ->with('finalAdjustmentPercent', $finalAdjustmentPercent)
            ->with('type', $type)
            ->with('units', $units)
    ...
    public function getProcess($id)
    {
        ...
        if (in_array($loggedInEmployeeId, $reportingLevel1EmployeeIds)) {
            ...
            // $append = ',T2.Level1Rating'; // old code

            $goal = DB::table('pms_employeegoal')
                ->select('Id', 'MultipleLevel1Appraiser')
                ->where('SysPmsNumberId', $pmsId)
                ->where('EmployeeId', $employeeId)
                ->first();

            $append = ', 0 as Level1Rating';

            if ($goal) {

                $query = DB::table('pms_employeegoaldetail as b')
                    ->where('b.EmployeeGoalId', $goal->Id);

                if ((bool) $goal->MultipleLevel1Appraiser) {
                ...
                $append = ', COALESCE(' . ($result->Level1Rating ?? 0) . ', 0) as Level1Rating';
            }
        ...
        return view('application.pmsprocess')
            ->with('goalAchievementScore', $goalAchievementScore)
            ->with('pmsMultiple', $pmsMultiple)
            ->with('loggedInLevel', $loggedInLevel)
            ->with('multipleDetailArray', $multipleDetailArray)
            ->with('hasMultipleLevel1', $hasMultipleLevel1)
            ->with('hasMultipleLevel2', $hasMultipleLevel2)
            ->with('history', $history)
            ->with('filePath4', $filePath4)
            ->with('empDetails', $empDetails)
            ->with('hasNoLevel2', $hasNoLevel2)
            ->with('details', $details)
            ->with('type', $type)
            ->with('id', $id)
            ->with('pmsDetails', $pmsDetails)
            ->with('pmsFile', $pmsFile)
            ->with('filePath2', $filePath2)
            ->with('filePath3', $filePath3);
    }
            ...
            if ($pmsCount > 0) {
                ...
                if ($withinFirstPMSOfYear) {
                    ...
                    $pmsHistory = DB::table("pms_submissionhistory as T1")
                        ...
                        ->whereRaw("T2.EmployeeId = ? and (DATE_FORMAT(T2.SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(T2.SubmissionTime,'%Y-%m-%d') <= ?)", [Auth::user()->Id, date(CONST_PMSSETTING_FIRSTPMSSTARTDATE), date(CONST_PMSSETTING_FIRSTPMSENDDATE)])
                    ...
                    $pmsDetails = DB::select("SELECT T1.Id,T1.SubmissionTime, T1.OfficeOrderPath, T2.Name as PMSOutcome, DATE_FORMAT(T1.OutcomeDateTime,'%D %M, %Y at %l:%i %p') as OutcomeDateTime, T1.LastStatusId, T1.StatusByEmployeeId, T1.PMSOutcomeId, T1.OfficeOrderEmailed,T1.FilePath, (select GROUP_CONCAT(CONCAT('<tr><td><strong><em>',DATE_FORMAT(A.StatusUpdateTime,'%D %M, %Y %l:%i %p'),':</strong></em></td><td><strong><em>',B.Name,'</strong></em></td>', '<td><strong><em>',C.Name,'</strong></em>', case when A.Remarks is not null and A.Remarks <> '' then concat('<br/><em>',A.Remarks,'</em>') else '' end,'</td></tr>') order by A.StatusUpdateTime SEPARATOR '<br/><br/>') from pms_submissionhistory A join mas_pmsstatus B on A.PMSStatusId = B.Id join mas_employee C on C.Id = A.StatusByEmployeeId where A.SubmissionId = T1.Id) as History from viewpmssubmissionwithlaststatus T1 left join mas_pmsoutcome T2 on T2.Id = T1.PMSOutcomeId where T1.EmployeeId = ? and (DATE_FORMAT(SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(SubmissionTime,'%Y-%m-%d') <= ?)", [Auth::user()->Id, date(CONST_PMSSETTING_SECONDPMSSTARTDATE), date(CONST_PMSSETTING_SECONDPMSENDDATE)]);
                    $pmsHistory = DB::table("pms_submissionhistory as T1")
                        ->join('pms_submission as T2', 'T2.Id', '=', 'T1.SubmissionId')
                        ->join('mas_employee as T3', 'T3.Id', '=', 'T1.StatusByEmployeeId')
                        ->join('mas_pmsstatus as T4', 'T4.Id', '=', 'T1.PMSStatusId')
                        ->leftJoin('mas_designation as T5', 'T5.Id', '=', 'T3.DesignationId')
                        ->whereRaw("T2.EmployeeId = ? and (DATE_FORMAT(T2.SubmissionTime,'%Y-%m-%d') >= ? and DATE_FORMAT(T2.SubmissionTime,'%Y-%m-%d') <= ?)", [Auth::user()->Id, date(CONST_PMSSETTING_SECONDPMSSTARTDATE), date(CONST_PMSSETTING_SECONDPMSENDDATE)])
                        ->orderBy('T1.StatusUpdateTime')
                        ->get(array('T1.StatusUpdateTime', 'T1.PMSStatusId', DB::raw("(select GROUP_CONCAT(concat(A.Name, ' (',C.Name,')') SEPARATOR ', ') from mas_employee A join mas_designation C on C.Id = A.DesignationId join pms_submissionmultiple B on B.AppraisedByEmployeeId = A.Id and B.ForLevel = 1 where B.SubmissionId = T1.SubmissionId) as Level1MultipleNames"), DB::raw("(select GROUP_CONCAT(concat(A.Name,' (',C.Name,')') SEPARATOR ', ') from mas_employee A join mas_designation C on C.Id = A.DesignationId join pms_submissionmultiple B on B.AppraisedByEmployeeId = A.Id and B.ForLevel = 2 where B.SubmissionId = T1.SubmissionId) as Level2MultipleNames"), 'T5.Name as LastStatusDesignation', 'T3.Id as StatusUpdatedById', DB::raw('T3.Name as StatusUpdatedBy'), 'T1.Remarks', 'T4.Name as Status'));
                }
            }
